...
def get_cate_type_0(headers,objece1,type_0):
    ...
    for i in tab_data:
        ...
        for brand in tab_list:
            ...
            tab_name = brand['name']
            # 每个品牌一个线程，多线程同时爬取
            t1=MyThread(run1_threading,args=(objece1,headers,brand,menu_name,tab_class,tab_name,))
            # t1=threading.Thread(target=run1_threading,args=(objece1,headers,brand,menu_name,tab_class,tab_name,))
            t1.start()
            list1.append(t1)
        for i in list1:
            i.join()
        for i in list1:
            df1=df1.append(i.get_result(),ignore_index=True)
    return df1


# 线程方法
def run1_threading(objece1,headers,brand,menu_name,tab_class,tab_name):
    df1 = pd.DataFrame()
    # 品牌名称
...
def get_cate_type_1_activy(headers,objece1,page_id,menu_name):
    ...
    for i in item_id:
        ...
        df1=df1.append(df2,ignore_index=True)
        print(f'{menu_name}-{page_id}-{i},获取完成')
        time.sleep(0.5)
    return df1
